...
bool Devices::setStatic(const std::string& color, uint32_t brightness) {
    bool ok = writeToFile(mBasePath + "run", 1);
    for (int segment = 0; segment < kSegmentCount; ++segment)
        ok &= writeToFile(mBasePath + "rgbcolor", std::to_string(segment) + " " + color);
    ok &= writeToFile(mBasePath + "brightness", brightness);
    return ok;
}
...
void Devices::setNotificationState(const State& state) {
    ...
    if (!state.color.isLit()) {
        ...
        writeToFile(mBasePath + "gradient", 0);
        writeToFile(mBasePath + "repeat", 0);
    ...
    if (blink) {
        ...
        ok &= setStatic(color.str(), brightness);
    }
    (void)ok;
}
